...
export class CalendarComponent implements OnInit {
  ...
  fillCalendar(_date: Date, item: EventModel[]) {
    ...
    const empty_d = { date: _date, day: 0, weekday: 0, event_desc: '' };
    this.event_list = [];
    ...
    for (let index = 1; index <= monthDays; index++) {
      const today = new Date(yyyy, mm - 1, index); // 抓今天星期幾。月份要減一，因為 js 的月份是 0 base
      const todayEvents = item.filter(x => new Date(x.WORK_DATE) >= today && new Date(x.WORK_DATE) <= today)
      ...
      if (todayEvents.length === 0) {
        ...
        event_desc = '<ul>' + todayEvents.join('') + '</ul>';
      }

      this.event_list.push({
        date: today, day: index, weekday: today.getDay(), event_desc: event_desc
      });
